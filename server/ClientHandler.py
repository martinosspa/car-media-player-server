...
class ClientHandler(threading.Thread):
	...
	def _check_if_client_is_fresh(self) -> None:
		...
		uuid_message = self._message_handler.receive_from_client(self._client)
		...
		if uuid_message == ComProt.UUID:
			...
			# Last Sync
			self._client_uuid = self._message_handler.receive_from_client(self._client)
			...
			self._message_handler.send_to_client(self._client, self._client_uuid)
	...
	def _handle_client_last_sync(self, client_uuid) -> None:
		...
		if float(client_last_sync) > float(self._client_data['clients'][client_uuid]['last_sync']):
			self._message_handler.send_to_client(self._client, ComProt.OKAY)
	...
	def _client_begin_sync(self, client_uuid) -> None:
		...
		self._message_handler.send_to_client(self._client, ComProt.SYNC)
		data = self._message_handler.receive_from_client(self._client)
		if data:
		...
		# End the handshake
		self._message_handler.send_to_client(self._client, ComProt.END)
	...
	def _send_file_differences_to_client(self, client_library) -> None:
		...
		for file in self._directory_manager.get_files():
			...
			if file[-1] == '/':
				...
				self._message_handler.send_to_client(self._client, file)
				time.sleep(0.05)
				continue

			self._message_handler.send_to_client(self._client, file)
			time.sleep(0.05)
			self._message_handler.send_file_to_client(self._client, file)
			
			# Wait for client message is a blocking method 
			# that waits for client to confirm that a file was received
			self._message_handler.wait_for_client_message(self._client, ComProt.SNF)
